 ...
 */
@WebServlet(name = "BackDeleteProdServlet",urlPatterns = {"/back/BackDeleteProdServlet"})
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try{
            ...
            response.getWriter().write("删除成功，2秒后跳转到商品列表，如果没有跳转请点击下面的链接跳转：</br>" +
                    "<a href="+request.getContextPath()+"/back/BackProdListServlet>跳转</a>");
        }catch(Exception e){
            response.getWriter().write("删除失败，2秒后跳转到商品列表，如果没有跳转请点击下面的链接跳转：</br>" +
                    "<a href="+request.getContextPath()+"/back/BackProdListServlet>跳转</a>");
        }
        response.setHeader("refresh","2;url="+request.getContextPath()+"/back/BackProdListServlet");
    }
}
